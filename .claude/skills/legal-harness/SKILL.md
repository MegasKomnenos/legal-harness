...
<important if="you are completing a step checkpoint, deciding which step to execute next, or considering whether existing outputs make a step unnecessary">
...
<important if="you are reading case documents in step 2 (2단계: 사건 문서 통독)">
...
<important if="you are reading court decisions for citation in step 3 (3단계: 판례 전문 통독)">
...
<important if="you are analyzing issues and presenting them to the user in step 5 (5단계: 쟁점 정리 및 방향 제시)">
...
<important if="you are designing the legal doctrine graph in step 6 (6단계: 법리 연결 그래프 설계)">
**유의사항**:
...
<important if="you are writing or editing the legal document in step 8 (8단계: 문서 생성)">
**그래프 준수 원칙**:
...
<important if="you are citing a court case (판례) or statute (법조문) in the legal document">
...
<important if="you are constructing legal arguments, refuting opposing claims, or applying legal doctrines to the facts of the case (포섭)">
...
<important if="you are drafting 청구취지, checking procedural scope, or applying doctrines to multiple claim items (항목별 포섭)">
...
<important if="you are writing a 정보공개청구서 세트 (별지, 법리보충 참고자료, or 요약본)">
...
<important if="you are writing a 행정심판 청구이유서 (administrative appeal statement of reasons)">
**행정심판 청구이유서**:
...
<important if="you are writing a 보충서면 (supplementary brief)">
**보충서면**:
...
<important if="you are writing a 국민신문고 민원 (civil petition via epeople.go.kr)">
**국민신문고 민원**:
- 톤: 정중하되 단호
- 전제 선언으로 민원의 성격 한정
...
| §17 항목별 개별 포섭 | 청구 대상이 여러 항목으로 구성된 경우, 각 항목에 대해 개별적으로 법리가 적용되었는가. 일반론 일괄 포섭으로 그치지 않았는가 |
| §18 쟁점 간 논리적 관계 | 주위적·예비적 관계에 있는 쟁점이 문서에서 명시되어 논리적 모순 없이 읽히는가 |
