...
1. **서브에이전트 제한**: 법률 문서의 탐색·읽기·생성에서 서브에이전트를 사용하지 않는다. 모든 탐색·읽기·생성을 메인 에이전트가 Read/Glob/Grep/Bash 도구로 직접 수행한다. **예외**: 7단계와 9단계의 Phase 2 내용 평가에 한하여 Agent 도구로 서브에이전트를 생성한다. 메인 에이전트의 확증 편향을 배제한 독립 평가를 위한 것이다.
...
1. 판례 인용 시 반드시 `data/판례_인용_사전.md`에 수록된 사건번호·선고일·판시사항 원문만 사용한다. 수록되지 않은 판례를 인용해야 하는 경우 "[미검증 판례]"로 표시한다.
2. 법조항 인용 시 조·항·호·목·단서의 번호를 정확히 기재하고, `data/법조문_인용_사전.md`에 수록된 조문 원문과 대조한다. 수록되지 않은 조문을 인용해야 하는 경우 "[미검증 조문]"으로 표시한다.
3. 생성 완료 후 `quality/검증_체크리스트.md`의 체크리스트로 자기 검증한다.
4. 문서 구조는 `style/문서_구조_템플릿.md`의 해당 유형 템플릿을 따른다.
5. 표현은 `style/표현_사전.md`의 정형 패턴을 활용한다.
6. 논증 구성 시 `style/논증_가이드라인.md`의 수정 전/수정 후 예시를 참조하여 동일한 오류를 반복하지 않는다.
7. 논증은 적극적으로 구성한다. "피청구기관이 ~을 제시하지 못하였다", "~을 밝히지 않았다"와 같이 상대방의 입증 부족만을 지적하는 소극적 논증으로 끝내지 않고, 왜 해당 요건이 충족되지 않는지를 사실관계와 법리에 기반하여 직접 논증한다.
8. 판례를 인용할 때는 판시사항뿐 아니라 해당 판결의 구체적 사실관계를 파악한 위에서, 이 사건의 사실관계와 어떻게 비교되는지를 밝힌다. 사실관계가 상이하여 적용이 어려운 판례는 인용하지 않는다.
9. 작성한 논지가 상대방에 의하여 역이용될 가능성이 없는지 점검한다.
10. 사건의 사실관계를 논증의 편의를 위하여 과소 또는 과대 서술하지 않는다.
11. 조문 용어와 판례 용어를 혼동하지 않는다. "고도의 개연성", "비교교량", "객관적으로 현저하게" 등은 판례에서 도출된 해석 기준이지 조문 자체의 문언이 아니다. 이러한 판례 용어를 사용할 때는 (가) 해당 판례를 먼저 인용한 뒤에 사용하고, (나) 주어를 판례로 명시한다("위 판결이 요구하는", "판례에 의하면"). 판례를 아직 인용하지 않은 맥락에서는 조문 문언을 사용한다.

#### 문서 유형별 생성 지침

**정보공개청구서 세트 (별지 + 법리보충 + 요약본)**:
- 생성 순서: 별지 -> 법리보충 -> 요약본
- 별지: 각 항목마다 "명시적으로 제외하는 부분" 필수. "본 청구는 ~ 요구하는 것이 아닙니다"로 한계 선언. 문서 명칭 한정 불가 선언 포함.
- 법리보충: 예상 비공개 사유별 선제적 방어. "피청구기관이 ~ 원용할 가능성이 있음을 인식" -> "그러나 ~" 패턴. 인용 법령·판례 목록을 문서 말미에 일괄 정리.
- 요약본: 별지·법리보충의 핵심만 압축. "본 요약본과 별지·참고자료 사이에 해석상 차이가 있는 경우에는 별지 및 참고자료의 기재가 우선합니다" 필수.
...
- 상대방이 원용하지 않은 불리한 판례를 먼저 꺼내지 않음
...
**Phase 1 탐지 대상** (단일 스크립트, 결정론적 패턴 매칭만):

금지 표현, AI 작문 특징, 비표준 용어, 소극적 논증 패턴, 역이용 위험 패턴, 번호 체계, 출처 혼동, 청구 범위 자발적 제약, 판례 용어 선행 인용 미검증, 미수록 판례/조문 인용, 재결 형식.

**Phase 2 서브에이전트 평가 기준** (`quality/검증_체크리스트.md`의 [Phase 2] 항목):

| 체크리스트 항목 | 점검 내용 |
|---------------|----------|
| §3 논증 완결성 | 대전제->소전제->결론의 3단 구조가 문맥상 완전한가 |
| §5 표현 적절성 | 정형 표현 패턴이 준수되었는가 |
| §6 제외 선언 | 포섭 구체성: 사실관계 적용이 구체적이며 과대·과소 서술이 없는가 |
| §8 부분공개 | 판례 인용 깊이: 사실관계 비교가 충분한가 |
| §11 논증 충실성 | 소극적 논증 문맥: 문단 전체가 적극적 논증으로 구성되었는가 |
| §12 판례 적정성 | 그래프-문서 정합성: 그래프의 모든 법리가 반영, 순서 일치 |
| §13 출처 구별 | 조문 문언과 판례 해석이 구별되었는가 |
| §15 그래프 정합성 | 그래프 JSON의 포섭·결론과 문서 논증이 일치하는가 |
| §16 작문 밀도 | 판례 과잉 나열 없음, 인용->포섭->결론 전환 자연스러움, 중복 논증 없음 |
...
- **표현**: `style/표현_사전.md` 정형 패턴 준수
- **구조**: `style/문서_구조_템플릿.md` 해당 유형 준수
- **PDF 서식**: `quality/PDF_서식.md` 확정 사양 준수
