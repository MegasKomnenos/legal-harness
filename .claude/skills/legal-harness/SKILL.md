...
이어서 작업 기억을 확인한다(`memory-harness` 핵심층 6 "읽기 경로"). `기억/색인.md`의 layer-3 절을 확인하고, 해당 사건의 현행 노드(L)와 이 작업 유형의 주제 노드(예: 보충서면이면 법률 논증 전략)를 전문 적재한다. 색인의 반영 수위 이후 layer-1·2 줄도 훑어 이 사건·작업과 관련된 노드가 있으면 선별해 읽는다(수위 이후분은 아직 현행 노드에 병합되지 않은 최신 기억이다). 무엇이 관련인지는 맥락으로 판단한다.

**[체크포인트]** 해당 사건 폴더에 `진행로그_{문서명}.md`를 생성하고 "### 1단계 완료"와 읽은 harness 파일 목록, 읽은 현행 노드(L) 목록을 기록한다.
...
3. 생성 완료 후 `quality/검증_체크리스트.md`로 자기 검증하되, 이 8단계 self-check는 **기계적·형식적 오류의 사전 선별로 한정**한다(인용 형식·번호 체계·표현 규칙 등). 논증의 근거 밀도·전략·표현 정밀성 같은 **판단적 품질의 실질 게이트는 9단계 Phase 2의 독립 서브에이전트**다. 생성 주체가 자기 산출을 판단하는 self-check는 확증편향에 취약하여(스스로 납득한 결론을 스스로 통과시킨다) 판단적 품질을 확정할 수 없으므로, 그 판단은 독립 검증에 맡긴다. self-check의 몫은 기계적 오류를 미리 걸러 9단계 검증 반복 횟수를 줄이는 데 있다.
...
15. 법적 글쓰기의 수준은 무엇을 주장하는가가 아니라 그 주장을 어떤 근거와 구조로 상대를 납득시키는가에서 갈린다. (가) 스스로 납득한 결론을 그대로 서술하지 않고, 그 결론에 이른 근거의 사슬을 문면에 전개하여 독자(재결청·법원)가 같은 결론에 이르도록 한다(수신자성). (나) 각 쟁점을 유리한 전제를 세우고(포석) 상대의 반박을 차단하며(봉쇄) 결론을 도출하는(관철) 구조로 구성한다(판 짜기). (다) 요건의 충족·불충족을 가르는 표현을 정밀하게 선택한다(표현 정밀성). 일반적 글쓰기 수준의 근거 밀도에 안주하는 것은 이 하네스가 요구하는 전문적 수준에 미치지 못한다 (상세: 논증_가이드라인 총칙 원리 A·B·C).
...
| 항목 | 설정 |
|------|------|
| **대상** | `cases/*/작성_최종/`·`작성_초안/`의 `.txt` 중 하네스 생성물(대응 진행로그·검증이력을 보유). `절차_*`·`증거`·`민원`·`분석`·`법적분석`·`외부소통`·`피청구인_증거`·`archive`의 수신·원본 자료는 검증 대상이 아니다 |
...
Phase 2는 8단계 self-check가 거르지 못하는 **판단적 논증 품질의 실질 게이트**다. 독립 서브에이전트는 체크리스트를 순회 확인하는 데 그치지 않고, 각 기준을 반증 태세로 적용하여 논증이 **이 사안의 맥락에서 적절한지**를 적대적으로 판단한다(고정 규칙의 기계적 대조가 아니다). 전략적 선택은 사안마다 다를 수 있으므로 "언제나 X"가 아니라 "이 사안에서 X가 적절한가"를 묻는다. 문서 유형에 특유한 전략(예: 정보공개 거부처분 청구인 보충서면의 공격 표면 최소화)은 `style/논증_가이드라인.md`의 해당 절을 참조해 사안 적합성을 판단한다. `validate_doc_eval.py`가 이 태세를 프롬프트에 반영한다.

최종 회차에서 전 항목 충족 후에만 최종본을 제시한다.

**사용자 첨삭의 결정화**: 검증 통과 후 또는 최종본 제시 후 사용자가 문서를 첨삭·반려·수정 지시하면, 그 첨삭을 단순 반영에 그치지 말고 `진행로그_{문서명}.md`에 구조화해 기록한다 -- ⓐ 시스템이 낸 기본값(거부된 것), ⓑ 사용자가 선호한 교정, ⓒ 그 간극이 시사하는 성문화 후보(생성·검증의 어느 파일·단계에서 미리 막을지, 짚이면). 사용자 첨삭 하나하나가 시스템에 대한 암묵적 진단이므로, 이 기록이 11단계 커밋에서 `memory-harness` 절차 1의 결정화(세칙 "사용자 교정 신호의 결정화")로 이어져 다음 dream이 프로세스 신호로 종합할 원재료가 된다. 다만 이 기록의 목적은 첨삭이 드러낸 판단을 규칙으로 고정하는 것이 아니라 신호를 포착하는 것이다. 무엇을 규범으로 성문화하고 무엇을 사안별 판단으로 남길지는 이후 종합·승인 단계의 몫이다(전략적 판단을 고정 규칙으로 동결하면 "언제나 그렇게 하라"는 잘못된 신호가 된다).
...
정리·보관이 끝나면 `.claude/CLAUDE.md`의 "커밋·푸시 절차"에 따라 이 문서 작업의 변경을 커밋한다. 한 세션에서 여러 문서를 생성한 경우 문서 단위로 분리 커밋할 수 있다. 사용자가 요청한 전체 작업이 끝나면 push한다.
