...
def ocr_image_bytes(img_bytes, ext="png"):
    tmp = SCRATCH_DIR / f"ocr_tmp.{ext}"
    tmp.write_bytes(img_bytes)
    text = ocr_image_file(tmp)
...
def extract_pdf(path):
    ...
    for i, page in enumerate(doc):
        ...
        if len(text) > 50:
            all_text.append(f"--- 페이지 {i+1} (텍스트 레이어) ---\n{text}")
        ...
        if has_content_img:
            ...
            if text:
                all_text.append(f"--- 페이지 {i+1} (텍스트 레이어, 부분) ---\n{text}")
                methods_used.add("pdf_text")
            ...
            all_text.append(f"--- 페이지 {i+1} (텍스트 레이어, 소량) ---\n{text}")
            methods_used.add("pdf_text")
